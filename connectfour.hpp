...
class ConnectFour {
    public:
        ...
        ConnectFour(const ConnectFour& other) : position{other.position}, mask{other.mask}, moveCounter{other.moveCounter} {}

        void playMove(bitboard move) {
            position ^= mask;
            mask |= move;
            moveCounter++;
        };      
        void playColumn(unsigned int column) {
            playMove((mask + bottomColumnMask(column)) & columnMask(column));
        };

        bitboard possibleMoves() const {
            return (mask + bottomMask) & boardMask;
        }
        void possibleColumns() const {

        };

        int currentPlayer() const { return moveCounter % 2; }
        bitboard yellow() const { return currentPlayer() == 1 ? position : position ^ mask; };
        bitboard red() const {  return currentPlayer() == 0 ? position : position ^ mask; };
        ...
        bitboard possibleNonLosingMoves() const {
            ...
            bitboard opponentWin = opponentWinningMoves();
        ...
        bool canWinNext() const {
            return winningMoves() & possibleMoves();
        }

        bitboard winningMoves() const {
